...
#define OPEN_FILE     (O_RDONLY)
#define CREATE_FILE   (O_CREAT | O_WRONLY)
#define MODE_DIR      (S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH)           // 0775
#define MODE_FILE     (S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH) // 0664

#define BUFFER_C_COMPAT(b) \
  do { \
...
int path_mkdir(Path* p) {
  ...
  do {
    ...
    if (exists) {
      ...
      // path does not exist
      if (mkdir(p->name.ptr, MODE_DIR) < 0) {
        ret = errno;
        break;
      }
    }
...
int path_touch(Path* p) {
  ...
  do {
    ...
    if (exists) {
      ...
      // path does not exist
      fd = open(p->name.ptr, CREATE_FILE | MODE_FILE);
      if (fd < 0) {
        ret = errno;
        break;
      }
...
static int write_to_file(Path* p, Slice s, int action) {
  ...
  int fd = -1;
  do {
    fd = open(p->name.ptr, CREATE_FILE | action, MODE_FILE);
    int err = fd < 0 ? errno : 0;

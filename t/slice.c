...
static void test_slice_empty(void) {
    ...
    cmp_ok(!!slice_empty(s), "==", !!1, "slice_empty(\"\")");
}
...
static void test_slice_integer(void) {
    static struct {
        const char* label;
        const char* s;
        int ret;
        ...
        { "empty"    , ""   , 0,  0 }, // an empty slice is not a valid int
        { "invalid 1", "x"  , 0,  0 },
        { "invalid 3", "xyz", 0,  0 },
        { "zero"     , "0"  , 1,  0 },
        { "nine"     , "9"  , 1,  9 },
    ...
    for (int j = 0; j < ALEN(data); ++j) {
        ...
        Slice s = slice_from_string(S, 0);
        long int val = 0;
        int ret = slice_integer(s, &val);
        ok(ret == data[j].ret, "slice_integer() returns %d = %d for %s OK", ret, data[j].ret, L);
        ok(val == data[j].val, "slice_integer() produces %d = %d for %s OK", val, data[j].val, L);
    }
}

static void test_slice_real(void) {
    static struct {
        const char* label;
        const char* s;
        int ret;
        ...
        { "empty"    , ""     , 0,  0.0 }, // an empty slice is not a valid double
        { "invalid 1", "x"    , 0,  0.0 },
        { "invalid 3", "xyz"  , 0,  0.0 },
        { "zero"     , "0.0"  , 1,  0.0 },
        { "nine"     , "9.3"  , 1,  9.3 },
        { "sixty six", "66.5" , 1, 66.5 },
    };
    ...
    for (int j = 0; j < ALEN(data); ++j) {
        ...
        Slice s = slice_from_string(S, 0);
        double val = 0;
        int ret = slice_real(s, &val);
        ok(ret == data[j].ret, "slice_real() returns %d = %d for %s OK", ret, data[j].ret, L);
        ok(val == data[j].val, "slice_real() produces %lf = %lf for %s OK", val, data[j].val, L);
    }
}
...
static void test_slice_find_byte(void) {
    ...
    for (int j = 0; j < ALEN(data); ++j) {
        ...
        if (e < 0) {
            cmp_ok(!!slice_empty(f), "==", !!1, "slice_find_byte([%s], [0x%02x]) => ABSENT", W, (unsigned int) B);
        } else {
...
static void test_slice_find_slice(void) {
    ...
    for (int j = 0; j < ALEN(data); ++j) {
        ...
        if (e < 0) {
            cmp_ok(!!slice_empty(f), "==", !!1, "slice_find_slice([%s], [%s]) => ABSENT", W, N);
        } else {
            int len = strlen(N);

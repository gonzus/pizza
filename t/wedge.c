...
static void test_char(void) {
    ...
    ok(w.b.len != 0, "wedge from char DOES use a buffer");
...
static void test_string(void) {
    ...
    ok(w.b.len == 0, "wedge from string does NOT use a buffer");
    ok(w.s.len == l, "wedge's slice has correct length of %d bytes", l);
...
static void test_ptr_len_with_null_correct_length(void) {
    ...
    ok(w.b.len == 0, "wedge from ptr/len with a null terminator does NOT use a buffer");
    ok(w.s.len == l, "wedge's slice has correct length of %d bytes", l);
    ok(w.s.ptr == s, "wedge's slice points to string [%s]", s);
    ok(w.s.ptr[w.s.len] == '\0', "wedge's slice is null terminated");
    wedge_destroy(&w);
...
static void test_ptr_len_with_null_too_long(void) {
    ...
    unsigned int l = strlen(s);
    Wedge w; wedge_build_from_ptr_len(&w, s, l);
    ok(w.b.len == 0, "wedge from ptr/len too long and a null terminator does NOT use a buffer");
    ok(w.s.len == l, "wedge's slice has correct length of %d bytes", l);
    ok(w.s.ptr == s, "wedge's slice points to string [%s]", s);
    ok(w.s.ptr[w.s.len] == '\0', "wedge's slice is null terminated");
    wedge_destroy(&w);
...
static void test_ptr_len_without_null(void) {
    ...
    Wedge w; wedge_build_from_ptr_len(&w, s, l);
    ok(w.b.len != 0, "wedge from ptr/len too short and no null terminator DOES use a buffer");
    ok(w.s.len == l, "wedge's slice has correct length of %d bytes", l);
...
static void test_buffer_without_null(void) {
    ...
    ok(w.b.len != 0, "wedge from buffer without a null terminator DOES use a buffer");
...
static void test_buffer_with_null(void) {
    ...
    Wedge w; wedge_build_from_buffer(&w, &b);
    ok(w.b.len == 0, "wedge from buffer with a null terminator does NOT use a buffer");
    ok(w.s.len == l, "wedge's slice has correct length of %d bytes", l);

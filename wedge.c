...
void build_from_slice(Wedge* w, Slice s, int null_terminated) {
    if (null_terminated) {
        ...
        w->s = slice_build_from_ptr_len(w->b->ptr, s.len);
    }
}
